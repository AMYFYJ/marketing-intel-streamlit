...
# Single source of truth for platform-valid creative formats, shared by sample
# generation and the goal planner's recommendation validation.
...
def sample_creative_by_platform(platform: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    """Pick creative formats that are valid for each platform."""
    ...
    unknown = pd.isna(creative)
    if unknown.any():
        creative[unknown] = rng.choice(np.array(CREATIVES, dtype=object), size=int(unknown.sum()))
    return creative
...
def recompute_metrics(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    df["cpa"] = _safe_divide(df["spend"], df["conversions"]).where(df["conversions"] > 0, df["spend"])
